...
function builder(yargs) {
    return yargs
        .usage(`\n${s.accent('do')} ${s.accent('json')} ${s.white('[src]')} ${s.white('[dest]')}\n\n${describe}`)
        ...
        .example(
            `${s.hilight('do json')} ${s.white('src/**/*.json dest/')} ${s.opt('--rc -p -o')}`,
            s.faded('Remove comments and uglify JSON files, (over)write to dest with parent dirs')
        )
        .example(
            `${s.hilight('do json')} ${s.opt('-c')} ${s.white('path/to/json.config.json')}`,
