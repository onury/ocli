...
async function copyOne(src, dest, options) {
    ...
    const { dot: ignored, errorOnExists, ...restOpts } = options; // node v8.6+  https://node.green/#ES2018-features-object-rest-spread-properties-object-rest-properties
    restOpts.preserveTimestamps = restOpts.timestamps;
    restOpts.errorOnExist = errorOnExists; // s at the end
    await fs.copy(src, dest, restOpts);
    return true;
}
...
const cmdOptionsMeta = {
    ...
    p: {
        alias: 'parents',
        describe: 'Also copy parent directory structure. Set to a number for levels.',
        // type: 'boolean', // or number (> 0)
        // default: false,
        global: false
    },
    o: {
...
function builder(yargs) {
    return yargs
        ...
        .example(
            `${s.hilight('o copy')} ${s.white('src/**/*.png dest/')} ${s.opt('-p -o')}`,
            s.faded('Copy PNG files, (over)write to dest with parent dirs')
        )
        .example(
            `${s.hilight('o copy')} ${s.opt('-c')} ${s.white('path/to/copy.config.json')}`,
